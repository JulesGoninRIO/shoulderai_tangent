...
def base_patient_id(patient):
    patient = str(patient)
    parts = patient.rsplit("-", 1)
...
def create_cross_validation_splits(df_split_labels, root_dir, n_splits):
    """Create k-fold cross-validation splits, grouping repeated scans like F0051 and F0051-2."""

    df = df_split_labels.copy()
    df["_base_patient"] = df["patient"].apply(base_patient_id)

    # Row-level availability
    has_three = (
        df["T1_sag"].notna()
        & df["T1_in_phase_sag"].notna()
        & df["T1_fs_cor"].notna()
    )
    has_t1_sag = df["T1_sag"].notna()

    # Base-patient category:
    # If any scan from the same base patient has all three image types,
    # put the whole base patient in the "three" category.
    # Else, if any scan has T1_sag, put the whole base patient in the T1_sag category.
    base_category = {}

    for base, group in df.groupby("_base_patient"):
        idx = group.index

        if has_three.loc[idx].any():
            base_category[base] = "three"
        elif has_t1_sag.loc[idx].any():
            base_category[base] = "t1_sag"
        else:
            base_category[base] = None

    df["_category"] = df["_base_patient"].map(base_category)

    three_bases = sorted(df.loc[df["_category"] == "three", "_base_patient"].unique())
    t1_sag_bases = sorted(df.loc[df["_category"] == "t1_sag", "_base_patient"].unique())

    fold_dfs = [pd.DataFrame() for _ in range(n_splits)]

    for category, base_patients in [
        ("three", three_bases),
        ("t1_sag", t1_sag_bases),
    ]:
        if len(base_patients) == 0:
            continue

        kf = KFold(n_splits=n_splits, shuffle=True, random_state=42)

        for fold, (train_idx, test_idx) in enumerate(kf.split(base_patients)):
            train_bases = {base_patients[i] for i in train_idx}
            test_bases = {base_patients[i] for i in test_idx}

            category_df = df.loc[df["_category"] == category].copy()

            category_df["phase"] = category_df["_base_patient"].apply(
                lambda base: "test" if base in test_bases else "train"
            )

            fold_dfs[fold] = pd.concat([fold_dfs[fold], category_df], ignore_index=True)

    for fold, fold_df in enumerate(fold_dfs):
        fold_df = fold_df.drop(columns=["_base_patient", "_category"])
        fold_df = fold_df.sort_values(by="patient").reset_index(drop=True)
...
def create_cross_validation_splits_old(df_split_labels, root_dir, n_splits): #name changed by Adrian
    """Create k-fold cross-validation splits.
