...
def save_masks_sag(img_types, patient_dir, patient, mask_dir, mask,dicom_dir):
    ...
    for img_type in img_types:
        ...
        if not os.path.isdir(os.path.join(dicom_dir, patient, img_type)):
            print(
                f"Skipping {patient}/{img_type}: annotation exists but matching DICOM sequence is missing"
            )
            continue
        ...
        if patient.startswith('A01') and patient !="A0103" or patient =="F0028" or patient == "A0098" or (patient == "F0015" and img_type=="T1_sag"):
            print(f"[MASK DEBUG] special annotation format triggered for patient={patient}, img_type={img_type}", flush=True)
            annotations = NewOsirixAnnotationList()
            patient_dir = os.path.join(dicom_dir, patient)
            print(f"[MASK DEBUG] dicom patient_dir={patient_dir}, exists={os.path.isdir(patient_dir)}", flush=True)
            images = list(set(os.listdir(patient_dir)) & set(img_types))

            print(f"[MASK DEBUG] patient={patient}, img_type={img_type}, "
                  f"available={os.listdir(patient_dir)}, expected_img_types={img_types}, "
                  f"matching={images}", flush=True)
