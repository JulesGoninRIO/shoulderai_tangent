...
class SegmentationModel(pl.LightningModule):
    ...
    def __init__(
        ...
        self.test_results_folder = str(Path(test_results_folder) / wandb_name)
    ...
    def forward(self, img):
        ...
        mask = self.model(img)
        #self.epoch_outputs = [] commented by Adrian, is called every batch not every epoch???
        return mask
    ...
    def shared_step(self, stage, batch, batch_idx):
        ...
        threshold = float(self.threshold_pred)
        preds = (probas_mask > threshold).float()

        preds_tensor = preds

        if self.mask == "tangent_sign" and stage == "test":
            preds_thin = np.array([
                skeletonize_mask(pred.detach().cpu().numpy())
                for pred in preds
            ])

            preds_tensor = torch.from_numpy(preds_thin).to(
                device=preds.device,
                dtype=torch.float32
            )
        ...
        if stage == "test":
            ...
            for i in range(len(img)):
                ...
                if self.mask == "tangent_sign":
                    ...
                    supra_mask = None
                    dice_mucle = None 
                    if idx is not None and not patient_name.endswith("aug"): #Adrian
    ...
    def calibrate_threshold_from_loader(self, dataloader): #Adrian
        """
        Compute one global threshold from a non-test dataloader.
        Use this on train or validation data, never on test data.
        """
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                # Use self.model directly to avoid side effects from forward()
                logits = self.model(img)
                probs = torch.nan_to_num(logits.sigmoid(), nan=0.0)

                all_probs.append(probs.detach().cpu().numpy().ravel().astype(float))
                all_masks.append(mask.detach().cpu().numpy().ravel().astype(float))
        ...
        if len(thresholds) == 0:
            ...
            best_idx = int(np.nanargmax(fscore[:-1]))
            best_threshold = float(thresholds[best_idx])

        self.threshold_pred = round(best_threshold, 4)

        print(f"Calibrated threshold: {self.threshold_pred}", flush=True)

        if was_training:
            self.train()

        return self.threshold_pred
