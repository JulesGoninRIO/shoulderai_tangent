...
class ROIDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            ...
            val_fold = (self.fold_index + 1) % self.k_folds

            val_split_path = os.path.join(
                self.root_dir,
                f"split_labels_fold_{val_fold}.csv"
            )

            # Get patient IDs belonging to validation fold
            ...
            # -----------------------------------------------------
            # Start from current fold's training pool
            # -----------------------------------------------------
            ...
            # Remove patients belonging to the validation fold
            self.train_indices = []

            for idx, filename in enumerate(train_data.dataset):
            ...
            # -----------------------------------------------------
            # Validation = next fold's test patients
            # -----------------------------------------------------
            val_data = ROIDataset(
                data_dir=self.data_dir,
                img_type=self.img_type,
                mask=self.mask,
                phase="test",
                split_labels_path=val_split_path,
                transform=self.test_transform,
                thicken_masks=True,
            )

            self.val_indices = list(range(len(val_data)))
...
class ROIDataset(Dataset):
 ...
 img_type, mask, phase, split_labels_path, transform=None, thicken_masks=True):
        ...
        if split_labels_path != None:
            ...
            self.patients = df_split_labels[
                df_split_labels["phase"] == phase
            ...
            if phase == "train":
                ...
                self.patients = original_patients + augmented_patients
        ...
        if split_labels_path is not None:
            dataset = sorted(
                set(self.patients)
                & set(os.listdir(self.img_dir))
